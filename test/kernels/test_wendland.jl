...
@testset "Wendland Kernel" begin
    ...
    @testset "ϕ_l polynomial expansion (l=$l)" for l in 1:11
        for x in rand(10)
            @test isapprox(ϕ_l(l)(x), (1 - x)^l, atol = 1e-8)
        end
    end
    ...
    @testset "Wendland Polynomials (d=$(1 + 2*d_iter))" for d_iter in 0:2
        ...
        @testset "k = $(k_iter-1)" for k_iter in eachindex(wendland_polys_d)
            ...
            for x in rand(10)
                @test cur_poly(x) ≈ WendlandPolynomial(d, k)(x) atol = 1e-8
            end
        end
    end
    ...
    @testset "Wendland Derivatives" begin
        ...
        @testset "d = $d, k=$k" for (d, k) in Iterators.product(1:2:5, 1:3)
            ...
            @testset "n = $n, m=$m" for (n, m) in Iterators.product(0:k, 0:k)
                ...
                @test 𝒟k𝒟′(lower, upper) ≈ 𝒟k𝒟′_approx(lower, upper) rtol = 1e-3 atol =
                    1e-8
                @test 𝒟k𝒟′(upper, lower) ≈ 𝒟k𝒟′_approx(upper, lower) rtol = 1e-3 atol =
                    1e-8
                @test 𝒟k𝒟′(lower, lower) ≈ 𝒟k𝒟′_approx(lower, lower) rtol = 2e-1 atol =
                    1e-6
                @test 𝒟k𝒟′(lower, out_of_support) ≈ 0 atol = 1e-12
            end
        end
    end
end